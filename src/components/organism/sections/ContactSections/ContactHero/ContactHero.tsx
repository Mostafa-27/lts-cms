...
interface ContactHeroFormData {
  title: string;
  subtitle: string;
  description: string;
}

const SECTION_ID = 16; // Contact Hero section ID

const ContactHero: React.FC = () => {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const { refreshPreview } = useSplitLayout();
  const { languages, selectedLangId, handleTabChange } = useSectionLanguage('16');
    ...
    const loadContent = async () => {
      ...
      try {
        ...
        if (content) {
          ...
          reset(contactDefaultData.ContactHero);
        }
      } catch (error) {
        console.error('Failed to load contact hero content:', error);
        reset(contactDefaultData.ContactHero);
      }
    };
    loadContent();  }, [selectedLangId, reset]);

  const onSubmit: SubmitHandler<ContactHeroFormData> = async (data) => {
    ...
    try {
      ...
      toast.success('Contact Hero section updated successfully!');
      refreshPreview();
    } catch (error) {
      console.error('Failed to update contact hero content:', error);
      toast.error('Failed to update Contact Hero section.');
    }
  };
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-4 bg-white dark:bg-gray-800 shadow rounded-lg">
                    ...
                    <div>
                      ...
                      <Controller
                        ...
                        render={({ field }) => <TextInput {...field} placeholder="Enter hero subtitle" />}
                      />
