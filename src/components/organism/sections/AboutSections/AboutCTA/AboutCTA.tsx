...
interface AboutCTAFormData {
  title: string;
  description: string;
}

const SECTION_ID = 25; // About CTA section ID
...
const AboutCTA: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<AboutCTAFormData> = async (data) => {
    ...
    try {
      ...
      toast.success('About CTA section updated successfully!');
      refreshPreview();
    } catch (error) {
      console.error('Failed to update about CTA content:', error);
      toast.error('Failed to update About CTA section.');
    }
  };
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-4 bg-white dark:bg-gray-800 shadow rounded-lg">
                    <div>
                      ...
                      <Controller
                        ...
                        render={({ field }) => <TextInput {...field} placeholder="Ready to Experience the LTS Difference?" />}
                      />
